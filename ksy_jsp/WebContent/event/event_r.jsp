				...
				<table class="table">
					...
					<thead>
						<tr>
							...
							<th scope="col">ApplyCondition</th>						
						</tr>
					</thead>
					<tbody>
<%	if ( total_count > 0 ){
...
%>				<a href="event_v.jsp">
					 <tr onClick="location.href='event_v.jsp?t_eventNo=<%=dtos.get(k).getEvent_no()%>'" style="cursor:pointer;">
						<td><%=k+1%></td>
						<td class="txt"><%=dtos.get(k).getTitle()%></a></td>
						<td class="gray"><span>
						...
						<td>
							<%=20+dtos.get(k).getStart_date()%>
						</td>
						<td>
							<%=20+dtos.get(k).getEnd_date()%>
						</td>
						<td><%=dtos.get(k).getReg_id()%></td>
						<td><%=dtos.get(k).getHit()%></td>
<%if(result>0){
%>						
						<td style="color:red">Submitted</td>
<%
}else {
%>
						<td>Not yet</td>
...
%>							
			...
			<a href="/event/event_w.jsp" class="btn-write">Write</a>
			<a href="/event/event_check.jsp" class="gagaga">ApplyList</a>
			<a href="/event/event_winner.jsp" class="gagaga">WinnerCK</a>
			<%
				} else {
				
			%>
				<a href="/event/event_winner.jsp" class="btn-write">WinnerCK</a>	
			<%
				}
			%>
		</div>
