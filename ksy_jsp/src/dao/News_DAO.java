...
public class News_DAO {
	...
	public int deleteNews(String news_no) { //공지사항 삭제
		int result = 0;	
		String query = " delete from A06_TRACK2_HOME_NEWS where news_no = '"+news_no+"' ";
		try {
			con = common.getConnection();
			ps  = con.prepareStatement(query);
			result  = ps.executeUpdate();
	...
	public int insertNews(News_DTO dto) { //뉴스 공지사항 등록
		int result = 0;
		String query = " insert into A06_TRACK2_HOME_NEWS "+
	...
	public String getMaxNo(){  //news_no 최대값 검사
		String query  = " select max(news_no) from A06_TRACK2_HOME_NEWS ";
		String result = null;
		try {
			con = common.getConnection();
			ps  = con.prepareStatement(query);
	...
	public int newsHit(String new_no){ //조회수 증가
		int result = 0;
		String query = " update A06_TRACK2_HOME_NEWS set hit = hit+1 where news_no ='"+new_no+"' ";
		try {
			con = common.getConnection();
			ps  = con.prepareStatement(query);
			result  = ps.executeUpdate();			
	...
	public News_DTO getNewsView(String new_no){ //상세조회
		...
		String query = " select a.news_no, a.title, a.content, a.file_name_1, b.name, to_char(a.reg_date, 'yy-MM-dd'), a.hit "+
					   " from A06_TRACK2_HOME_NEWS a, A06_TRACK2_HOME_MEMBER b "+
					   " where a.reg_id = b.id "+
